    ...
    def parse(self, response):
        for row in response.xpath('/html/body/div/div[2]/main/section/div[2]/table[2]/tbody[1]/tr'):
            yield {
                ...
                'gold': row.xpath('td[3]//text()').extract_first(),
                'silver': row.xpath('td[4]//text()').extract_first(),
                'bronze': row.xpath('td[5]//text()').extract_first(),
                'total': row.xpath('td[6]//text()').extract_first(),
            }
